...
uniform vec3 lightEyeDirection;

uniform vec3 lightColor;
uniform float lightIntensity;

uniform float cosineCutoff;
uniform float edgeDimming;
...
void main(){
	...
	if(diffuse > 0.0){
		...
		float coneFactor = pow(max(0, -dot(L, lightEyeDirection) - cosineCutoff)/(1 - cosineCutoff), edgeDimming);
		
		color = lightColor * (diffuseColor + specular) * (diffuse * falloff*lightIntensity*coneFactor * shadowCone(eyeSpace.xyz));
	}
	light = color;
